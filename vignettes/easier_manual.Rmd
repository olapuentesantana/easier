...
In order to use `easier` in your study, bulk-tumor RNA sequencing data is required as input (when available, patient response to immunotherapy can be optionally entered):
...
In this vignette, we will illustrate the main features of `easier` on a publicly available melanoma dataset including both response to ICB therapy and RNA sequencing from Gide, et al. "Distinct Immune Cell Populations Define Response to Anti-PD-1 Monotherapy and Anti-PD-1/Anti-CTLA-4 Combined Therapy", published in Cancer Cell, 2019 [doi:10.1016/j.ccell.2019.01.003](https://doi.org/10.1016/j.ccell.2019.01.003). ** Still need to ask for consent.

The RNA sequencing fastq files are available from NCBI BioProject [PRJEB23709](https://www.ncbi.nlm.nih.gov/bioproject/522397). For the example dataset, we considered 41 metastatic melanoma patients () treated with anti-PD1 with prior BRAFi therapy.
...
```{r eval = FALSE}
install.packages("/Users/Oscar/ownCloud2/SystemsImmunoOncology/easier_project/easier_0.9.0.tar.gz", repos = NULL, type="source")
library(easier)
````

If you have your bulk-tumor RNA sequencing data ready, you can compute five quantitative descriptors of the tumor microenvironment.
