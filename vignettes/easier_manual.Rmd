...
Along this vignette we will illustrate the main features of `easier` on a publicly available bladder cancer dataset including both response to ICB therapy and RNA sequencing from Mariathasan et al. "TGF-B attenuates tumour response to PD-L1 blockade by contributing to exclusion of T cells", published in Nature, 2018 [doi:10.1038/nature25501](https://doi.org/10.1038/nature25501).

The RNA sequencing data of Mariathasan cohort is made available via `IMvigor210CoreBiologies` package. Please follow  [IMvigor210CoreBiologies](http://research-pub.gene.com/IMvigor210CoreBiologies/) vignette to install and load `IMvigor210CoreBiologies` package, so the data from Mariathasan cohort can be accessible by `easier`. Here, we will consider bladder cancer patients with complete response (CR) as responders and patients with progressive disease (PD) as non-responders.
...
To download and install `IMvigor210CoreBiologies` package including its dependencies, we followed [IMvigor210CoreBiologies](http://research-pub.gene.com/IMvigor210CoreBiologies/) vignette.
...
Once `IMvigor210CoreBiologies` package is loaded, we can retrieve the RNA sequencing data from Mariathasan cohort as follows:
...
We recommend the user to create a dataset object as described below:

```{r eval = FALSE}
# Create a external dataset object
setClass("DatasetUser", slots=list(name="character", counts="data.frame", tpm="data.frame", response="character", TMB="numeric", cancertype="character"))

obj_dataset <- new("DatasetUser",
                   name="Mariathasan", 
...
cell_fractions <- compute_cell_fractions(RNA_tpm = obj_dataset@tpm)

# Obtain pathway scores
pathway_activity <- compute_pathways_scores(RNA_counts = obj_dataset@counts,
                                            remove_genes_ICB_proxies = TRUE)

# Obtain TF activity
tf_activity <- compute_TF_activity(RNA_tpm = obj_dataset@tpm, 
                                   remove_genes_ICB_proxies = FALSE)

# Obtain ligand-receptor pair weights
lrpair_weights <- compute_LR_pairs(RNA_tpm = obj_dataset@tpm, 
                                   remove_genes_ICB_proxies = FALSE, 
                                   cancer_type = "pancan")
...
tasks_values <- compute_gold_standards(RNA_tpm = obj_dataset@tpm, 
...
predictions_immune_response <- predict_immune_response(pathways = pathway_activity,
                                                       ...
                                                       cancer_type = obj_dataset@cancertype, 
                                                       verbose = TRUE)
...
Additionally the user can set `combo_easier_and_TMB = TRUE` to apply a refined approach based on the integration of `easier` predictions with information on tumor mutational burden (TMB). 
...
assess_immune_response(predictions_immune_response = predictions_immune_response,
                       real_patient_response = obj_dataset@response,
                       RNA_tpm = obj_dataset@tpm,
                       output_file_path = "../figures",
                       cancer_type = obj_dataset@cancertype,
                       TMB_values = obj_dataset@TMB,
