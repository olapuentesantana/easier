...
if (anyDuplicated(newnames_dup)){
  ...
  tmp2 <- aggregate(tmp, by = list(dup_genes), FUN = "mean")
...
The option `patient_label` allows to compare two different conditions by providing a character string involing two factor levels, for instance, patients' clinical response (where non-responders are labeled as NR and responders as R). 
