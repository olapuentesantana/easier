...
This vignette describes how to use the easier package for a pleasant onboarding experience around the EaSIeR approach. EaSIeR is a tool to predict biomarker-based immunotherapy response providing just the patients' RNA-sequencing (RNA-seq) data as input. 
...
Leveraging the data from The Cancer Genome Atlas (TCGA), regularized multi-task linear regression was used to identify how these descriptors can simultaneously predict several state-of-the-art hallmarks of anti-cancer immune response (see table below). Here, the regularization term is used to select features that are relevant for all tasks.
...
- `compute_pathways_scores` infers pathway activity from raw counts gene expression using PROGENy method [@HOLLAND2020194431; @Schubert2018].
...
- `compute_CC_pairs_grouped` scores cell-cell interactions in the TME using ligand-receptor weights as input.
- `compute_gold_standards` computes the published transcriptomics-based hallmarks of anti-cancer immune response (so-called gold standards) as established by the user.
...
tasks <- c("CYT", "IPS", "IMPRES", "Roh_IS", "chemokines", "Davoli_IS", "IFNy",
           "Ayers_expIS", "Tcell_inflamed", "TIDE", "MSI", "TLS", "CTLA4", "PD1", "PDL1")
...
cancertype <- "coadread"
output_file_path <- "ars_dir"
#compute_gold_standards(RNA.tpm = tpm,list_gold_standards = tasks,cancertype,output_file_path)
#selected_signatures <- c("CYT", "IMPRES", "Roh_IS", "chemokines", "Davoli_IS", "IFNy",
# "Ayers_expIS", "Tcell_inflamed", "MSI", "TLS", "ICB_genes", "RIR", "TIDE", "IPS")
selected_signatures <- c("CYT", "IMPRES", "Roh_IS", "chemokines", "Davoli_IS", "IFNy",
 "Ayers_expIS", "Tcell_inflamed", "MSI", "TLS", "ICB_genes")
easier::compute_gene_signatures(tpm, selected_signatures)
```
