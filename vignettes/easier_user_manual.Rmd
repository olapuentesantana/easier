...
<center>
![](/Users/Oscar/ownCloud2/SystemsImmunoOncology/easier_project/easier_image.png)
</center>  
...
- Predict biomarker-based immunotherapy response
...
# patient response
patient_ICBresponse <- dataset_mariathasan@colData$BOR; names(patient_ICBresponse) <- dataset_mariathasan@colData$pat_id
...
## Derive trasncriptomic-based scores associated with different hallmarks of anti-tumor immune responses
...
## Obtain quantitative descriptors of the TME {#features}
...
## Get patients' predictions of anti-tumor immune responses {#predict}
...
## Evaluate easier predictive performance of patients' immunotherapy response {#evaluation}
...
Evaluation of patients' immunotherapy response can be done via `assess_immune_response` function:

```{r, eval=TRUE}
output_evaluation <- assess_immune_response(predictions_immune_response = predictions_immune_response,
...
The output `assess_immune_response` returns two figures informing about the accuracy of easier predictions on patients' response to ICB therapy.

  - A first figure with receiver operating characteristic (ROC) curves based on quantitative descriptors of the TME, an ensemble descriptor based on average of individual descriptors, and the computed scores of immune response (gold standard). ROC curves were computed as the average of the ROC curves obtained for each score of immune response.
  - A second figure with Area under the curve (AUC) values represented as bar plots displaying the average AUC across tasks and including error bars to describe the corresponding standard deviation.
...
To deal with this, assigning each patient an score of likelihood of immune response can be easily done by using the function `assess_immune_response`, but now the option `patient_response` should be ommited as follows:
