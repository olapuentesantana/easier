...
compute_MSI <- function(RNA_tpm) {
  ...
  MSI.matrix <- matrix(0, nrow(MSI.basis), ncol(RNA_tpm)) ; colnames(MSI.matrix) <- colnames(RNA_tpm)
  remove_pairs <- vector("list", length = ncol(RNA_tpm)) ; names(remove_pairs) <- colnames(RNA_tpm)
