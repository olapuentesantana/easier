...
#' @examples
#' # TODOTODO
compute_ICB_genes <- function(RNA_tpm) {

  # Extract position genes for GZMA and PRF1
  tmp <- match(c("CD274","CTLA4","PDCD1"), rownames(RNA_tpm))

  # PDL-1 calculation
  PDL1_expr = RNA_tpm[tmp[1],] ; rownames(PDL1_expr) <- "PDL1"

  # CTLA-4 calculation
  CTLA4_expr = RNA_tpm[tmp[2],] ; rownames(CTLA4_expr) <- "CTLA4"

  # PD-1 calculation
  PD1_expr = RNA_tpm[tmp[3],] ; rownames(PD1_expr) <- "PD1"
